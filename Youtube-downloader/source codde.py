...
def Click():
    ...
    try:
        ...
        m_box.showerror("Error", "There is a Connection Problem !")
    else:
        vid = yt.streams.get_highest_resolution()
        destination = str(got_path)
        vid.download(destination)
        os.startfile(got_path)
        return m_box.showinfo('Successfully Downloaded.', f"Your YouTube Video is Downloaded Successfully at {got_path}/{yt.title}")
...
def startThredProcess():
    myNewThread = threading.Thread(target=Click)
